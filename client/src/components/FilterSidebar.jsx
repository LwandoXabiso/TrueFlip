...
const FilterSidebar = ({showFilterPhone, setShowFilterPhone, filters, setFilters}) => {
    ...
    const platforms = [
        ...
        {value: "instagram", label : "Instragram"},
  ...
  return (
    <div className={`${showFilterPhone ? "max-sm:fixed" : "max-sm:hidden"} max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-none shadow-sm border border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}> 
      <div className='p-4 border-b border-gray-200'>
        <div className='flex items-center justify-between'>
                <div className='flex items-center space-x-2 text-gray-700'>
                    <Filter className='size-4'/>
                    <h3 className='font-smeibold'>Filters</h3>
                </div> 
                <div className='flex items-center gap-2'>

                    <X onClick={onClearFilters} className='size-6 text-gray-500 hover:text-gray-700 p-1 hover:bg-gray-100 rounded-none transition-colors cursor-pointer' />

                    <button onClick={()=>setShowFilterPhone(false)} className='sm:hidden text-sm border text-gray-700 px-3 py-1 rounded-none'>Apply</button>
                </div>
        </div>
      </div>
      ...
      <div className='p-4 space-y-6 sm:max-h-[calc(100vh-200px)] overflow-y-auto no-scrollbar'>
        ...
        {/* Search Bar */}
        <div className='flex items-center justify-between'>
            <input type='text' placeholder='Search by username, platform, etc..' className='w-full text-sm px-3 py-2 border border-gray-300 rounded-md outline-gray-500' onChange={onChangeSearch} value={search}/>
        </div>
        {/* Platform Filter */}
        <div>
        ...
        <div>
            ...
            {expandedSections.price && (
                <div className='space-y-3'>
                    ...
                    className='w-full h-2 bg-gray-200 rounded-none apperarance-none cursor-pointer accent-gray-600'/>
        ...
        <div>
            ...
            {expandedSections.followers && (
                ...
                className='w-full px-3 py-2 border border-gray-300 rounded-none text-gray-700 outline-gray-500'>
